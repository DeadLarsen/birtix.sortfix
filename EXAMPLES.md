...
- [Быстрый старт после Composer](#быстрый-старт-после-composer)
...
## 🚀 Быстрый старт после Composer

### После установки `composer require deadlarsen/iblocksortfix`

Выполните следующую последовательность команд для начала работы:

```bash
# 1. Проверьте текущее состояние системы
php local/modules/deadlarsen.iblocksortfix/cli/sort_fix.php stats

# 2. Определите нужно ли исправление
php local/modules/deadlarsen.iblocksortfix/cli/sort_fix.php check

# 3. Если нужно исправление - создайте бекап
...
Если вы хотите быстро исправить сортировку:

```bash
# Создать бекап и исправить сортировку одной командой
php local/modules/deadlarsen.iblocksortfix/cli/sort_fix.php fix --backup
```
